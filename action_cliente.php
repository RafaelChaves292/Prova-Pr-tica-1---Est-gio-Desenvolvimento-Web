...
//			$data_ansi = $data_temp[2] . '/' . $data_temp[1] . '/' . $data_temp[0];
		...
		if ($acao == 'incluir'):
			...
			$sql = 'INSERT INTO tab_cliente (nome, endereco, numero, bairro, cidade, uf, cep, cpf, email)
							   VALUES(:nome, :endereco, :numero, :bairro, :cidade, :uf, :cep, :cpf, :email)';
		...
		if ($acao == 'editar'):
			...
			$sql = 'UPDATE tab_cliente SET nome=:nome, endereco=:endereco, numero=:numero, bairro=:bairro, cidade=:cidade, uf=:uf, cep=:cep, cpf=:cpf, email=:email  ';
			$sql .= 'WHERE id = :id';
